 ...
 *
 * 本接口仅在商户入驻申请被驳回后使用；
 * 调用前提条件：审核状态为「入驻驳回」时可调用；
 * 调用后重新提交全部商户信息，进入新一轮审核流程。
 */
class UpdateMerchantInfoRequest extends Request
{
    // --------------------------------------基础信息--------------------------------------
     ...
     * 05 事业单位
     * 06 其他组织
     * mecTypeFlag=01（连锁总店）时，此处不能选自然人；
     ...
     * 01 连锁总店
     * 02 连锁分店
     * mecTypeFlag=02（连锁分店）时需校验所属总店
     * mecTypeFlag必须为01 连锁总店
     */
     ...
     * 复核回调地址.
     *
     * 传入则在商户入网状态为【入驻通过】时，返回复核标识及补充资料内容推送至该地址
     */
     ...
     * 结算账户名.
     *
     * 对公结算账户名需与注册名有互相包含关系；自然人商户、线上类商户不允许授权结算
     */
    public ?string $actNm;
     ...
     * 其他资料照片1地址.
     *
     * 机构跟银行的合作协议关键页或机构银行合作授权函（支付宝非盈利时需要上传）
     */
    ...
    protected string $uri = '/merchant/updateMerchantInfo';
